...
const svg = (glyphTransform = "translate(0.5 0)") =>
  ...
  `<path d="${GLYPH}" transform="${glyphTransform}" fill="${SIGNAL}" fill-rule="evenodd"/>` +
  `</svg>\n`;
...
const svgSmall = (px) =>
  ...
  `<path d="${GLYPH_16}" fill="${SIGNAL}" fill-rule="evenodd"/>` +
  `</svg>\n`;
...
magick(
  ...
  "-fill", SIGNAL, "-pointsize", "37", "-annotate", "+194+154", "LOWBED · GABARİ DIŞI AĞIR TAŞIMACILIK · DİYARBAKIR",
  // telefon plakası
  "-fill", "#0F1115E6", "-draw", "rectangle 792,520 1164,606",
  "-fill", SIGNAL, "-draw", "rectangle 792,520 798,606",
  "-fill", "#A7AFB8", "-pointsize", "24", "-annotate", "+818+552", "7/24 OPERASYON HATTI",
