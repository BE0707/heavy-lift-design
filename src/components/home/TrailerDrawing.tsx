...
const Text = ({ x, y, children, anchor = "start", fill = C.line, size = 12, rotate }: {
...
const Wheel = ({ cx, r, phantom = false }: { cx: number; r: number; phantom?: boolean }) => (
  <g>
    <circle cx={cx} cy={GROUND - r} r={r} fill={phantom ? "none" : C.ground} stroke={phantom ? C.muted : C.line} strokeWidth={1.5} strokeDasharray={phantom ? "4 3" : undefined} />
...
const Tractor = ({ x }: { x: number }) => (
  <g>
    <rect x={x - 52} y={GROUND - 50} width={278} height={12} fill={C.body} stroke={C.line} strokeWidth={1.5} />
...
const Frame = ({ id, view, title, children }: { id: string; view: string; title: string; children: ReactNode }) => (
  <svg viewBox="0 0 960 300" role="img" aria-labelledby={`${id}-title`} className="block h-auto w-full">
    ...
    <Defs id={id} />
    <Text x={24} y={28} fill={C.muted} size={11}>{`${view} · ŞEMATİK · ÖLÇEKSİZ`}</Text>
    {children}
  </svg>
);
...
const Envelope = ({ x, y, w, h, label }: { x: number; y: number; w: number; h: number; label: string }) => (
  <g>
    ...
    <Text x={x + 10} y={y + 20} fill={C.muted} size={11}>{label}</Text>
  </g>
);
...
const Standard = () => {
  ...
  return (
    <Frame id={id} view="YAN GÖRÜNÜŞ" title="Standart lowbed şematik yan görünüş: platform yüksekliği yaklaşık 0,9–1,0 m, platform uzunluğu yaklaşık 9,5–10,5 m, 3 veya 4 dingil, 4,00 m yasal yükseklik sınırı.">
      ...
      {/* rampa (kaldırılmış) */}
      <polygon points="175,210 189,210 172,148 160,148" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      ...
      {/* platform + kaz boynu */}
      <rect x="175" y="210" width="382" height="10" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <polygon points="549,210 576,180 590,193 562,220" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="576" y="180" width="146" height="14" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <Wheel cx={215} r={15} />
      <Wheel cx={267} r={15} />
      <Wheel cx={319} r={15} />
      ...
      <Tractor x={690} />
      <Ground id={id} />
      <Text x={293} y={270} anchor="middle" fill={C.muted} size={11}>3 (+1) DİNGİL</Text>
...
const Well = () => {
  ...
  return (
    <Frame id={id} view="YAN GÖRÜNÜŞ" title="Havuzlu çok dingilli dorse şematik yan görünüş: havuz yüksekliği yaklaşık 0,4–0,5 m, havuz uzunluğu yaklaşık 7–9 m, 5–8 dingil, 4,00 m yasal yükseklik sınırı.">
      ...
      <rect x="150" y="212" width="242" height="10" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <polygon points="388,212 402,212 414,232 400,232" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="400" y="232" width="182" height="8" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <polygon points="576,232 603,180 617,193 590,240" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="603" y="180" width="119" height="14" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      {[172, 200, 228, 256, 284].map((cx) => <Wheel key={cx} cx={cx} r={12} />)}
      {[312, 340, 368].map((cx) => <Wheel key={cx} cx={cx} r={12} phantom />)}
      <Tractor x={690} />
      <Ground id={id} />
      <Text x={270} y={272} anchor="middle" fill={C.muted} size={11}>5–8 DİNGİL</Text>
...
const Telescopic = () => {
  ...
  return (
    <Frame id={id} view="YAN GÖRÜNÜŞ" title="Teleskopik uzatmalı lowbed şematik yan görünüş: kapalı platform yaklaşık 13,6 m, uzatılmış yaklaşık 24 m'ye kadar; arkada çıkıntılı uzun yük.">
      {/* uzun yük (kiriş) */}
      <rect x="44" y="176" width="498" height="32" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <Text x={293} y={197} anchor="middle" fill={C.muted} size={11}>KİRİŞ / BORU / KULE ELEMANI</Text>
      <rect x="30" y="176" width="14" height="14" fill={`url(#${id}-hatch)`} stroke={C.limit} />
      <Text x={30} y={166} fill={C.limit} size={11}>ARKA ÇIKINTI</Text>
      {/* uzatma bölgesi (iç kiriş) + ana kiriş */}
      <rect x="190" y="211" width="142" height="6" fill="none" stroke={C.muted} strokeDasharray="4 3" />
      <rect x="60" y="208" width="130" height="10" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="332" y="208" width="225" height="10" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <polygon points="549,208 576,180 590,193 562,218" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="576" y="180" width="146" height="14" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <Wheel cx={88} r={15} />
      <Wheel cx={128} r={15} />
      <Wheel cx={168} r={15} />
      <Tractor x={690} />
      <Ground id={id} />
      <Text x={261} y={236} anchor="middle" fill={C.muted} size={11}>← UZATMA →</Text>
...
const Escort = () => {
  ...
  const car = (x: number, label: string) => (
    <g>
      <rect x={x} y={177} width={90} height={36} fill={C.body} stroke={C.line} strokeWidth={1.5} />
  ...
  return (
    <Frame id={id} view="PLAN GÖRÜNÜŞ" title="Eskort düzeni şematik plan görünüş: önde ve arkada refakat aracı, ortada gabari dışı genişlikte yük taşıyan çekici ve lowbed.">
      ...
      <Text x={24} y={112} fill={C.muted} size={11}>KARŞI ŞERİT</Text>
      {car(60, "ARKA ESKORT")}
      {/* dorse + gabari dışı yük */}
      <rect x="220" y="170" width="330" height="50" fill="none" stroke={C.line} strokeWidth={1.5} />
      <rect x="250" y="163" width="280" height="64" fill={C.body} stroke={C.limit} strokeWidth={1.5} />
      <Text x={390} y={200} anchor="middle" fill={C.muted} size={11}>GABARİ DIŞI YÜK</Text>
      {/* çekici */}
      <rect x="556" y="172" width="98" height="46" fill={C.body} stroke={C.line} strokeWidth={1.5} />
      <rect x="628" y="174" width="26" height="42" fill="none" stroke={C.muted} />
      <Text x={440} y={150} anchor="middle">ÇEKİCİ + LOWBED</Text>
      {car(760, "ÖN ESKORT")}
      <path d="M880 250 H930" stroke={C.dim} strokeWidth={1.5} markerEnd={`url(#${id}-arrow)`} />
      <Text x={876} y={254} anchor="end" fill={C.muted} size={11}>SEYİR YÖNÜ</Text>
