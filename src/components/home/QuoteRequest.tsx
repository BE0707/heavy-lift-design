    ...
    <section id="fiyat-talebi" aria-labelledby="fiyat-title" className="border-b border-rule py-20 lg:py-28">
      <div className="container">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-12 lg:gap-10">
          <form noValidate onSubmit={onSubmit} className="border border-rule-strong bg-graphite lg:col-span-7" aria-labelledby="fiyat-title">
            <div aria-live="assertive">
              {errorCount > 0 && (
                <p className="border-b border-hazard bg-hazard/10 px-5 py-3 text-sm text-bone sm:px-7">
                  Formda {errorCount} alan eksik veya hatalı. İşaretli alanları kontrol edin.
                </p>
              )}
            </div>

            <fieldset className="grid gap-5 border-b border-rule-strong px-5 py-6 sm:grid-cols-2 sm:px-7">
              <legend className="label float-left mb-1 w-full text-dim sm:col-span-2">01 · Yük</legend>
              ...
              <div className="sm:col-span-2">
                <p className="mb-2 flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                  <span className="font-display text-[1.0625rem] font-semibold uppercase tracking-[0.03em] text-bone">Ölçüler (metre)</span>
                  <span className="label">Ön kontrol için önerilir</span>
                </p>
                <div className="grid grid-cols-3 gap-3">
                  {(
                    ...
                    <div key={key}>
                      <label htmlFor={`q-${key}`} className="label mb-1.5 block">
                        {label}
                      </label>
                      <input
                        className="field tabular font-mono"
            ...
            <fieldset className="grid gap-5 border-b border-rule-strong px-5 py-6 sm:grid-cols-2 sm:px-7">
              <legend className="label float-left mb-1 w-full text-dim sm:col-span-2">02 · Güzergah</legend>
            ...
            <details ref={detailsRef} className="group border-b border-rule-strong">
              <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between px-5 font-display text-[1.0625rem] font-semibold uppercase tracking-[0.03em] text-bone hover:bg-plate sm:px-7 [&::-webkit-details-marker]:hidden">
                03 · Ek bilgiler
                <span aria-hidden="true" className="font-mono text-lg text-steel transition-transform group-open:rotate-45">+</span>
              </summary>
              <div className="grid gap-5 px-5 pb-6 pt-1 sm:grid-cols-2 sm:px-7">
            ...
            <div className="grid gap-5 px-5 py-6 sm:px-7">
              <fieldset>
                <legend className="mb-2 font-display text-[1.0625rem] font-semibold uppercase tracking-[0.03em] text-bone">Mesaj gidecek hat</legend>
                <div className="grid gap-2 sm:grid-cols-2">
                  {DISPATCHERS.map((d) => (
                    <label
                      key={d.id}
                      className="flex min-h-14 cursor-pointer items-center gap-3 border border-rule-strong bg-ink px-4 py-3 transition-colors hover:border-steel has-[:checked]:border-signal has-[:focus-visible]:outline has-[:focus-visible]:outline-[3px] has-[:focus-visible]:outline-offset-[3px] has-[:focus-visible]:outline-signal"
                    >
                      <input
                        type="radio"
                        name="dispatcherId"
              ...
              <div role="status">
                {sentUrl && (
                  <p className="border-l-2 border-signal pl-3 text-sm text-steel">
